...
class NetworkPort(HostHelpersBase):
    ...
    def cache_load(self, plugin=False):
        if plugin:
    ...
    def cache_save(self, data, plugin=False):
        if plugin:
            cache_root = self.plugin_cache_root
        else:
            cache_root = self.global_cache_root
    ...
    def stats(self):
        ...
        counters = self.cache_load(plugin=False) or {}
        if counters:
            return counters
        ...
        if counters:
            self.cache_save(counters, plugin=False)
            return counters
...
class HostNetworkingHelper(HostHelpersBase):
    ...
    def cache_load(self, namespaces=False, plugin=False):
        if plugin:
            cache_root = self.plugin_cache_root
        else:
            cache_root = self.global_cache_root
    ...
    def cache_save(self, data, namespaces=False, plugin=False):
    ...
    def _get_interfaces(self, namespaces=False):
        ...
        self.cache_save(interfaces_raw, namespaces=namespaces, plugin=False)
        for iface in interfaces_raw:
            interfaces.append(NetworkPort(**iface))
